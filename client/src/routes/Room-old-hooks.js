...
const Room = (props) => {
  ...
  function createSteam(stream) {
    ...
    socketRef.current.on("disconnected", (userID) => {
      ...
      console.log("partnerVideos.current :>> ", partnerVideos.current);
      partnerVideos.current = partnerVideos.current.filter(
    ...
    userStream.current
      ...
      .forEach((track) =>
        senders.current.push(
          peerRef.current[userID].addTrack(track, userStream.current)
        )
      );
  }
  ...
  function handleAnswer(message) {
    ...
    console.log(
      "2. handleAnswer peerRef.current :>> ",
      peerRef.current[userID]
    );
  ...
  function handleICECandidateEvent(e, userID) {
    console.log(
      "2.1 handleICECandidateEvent otherUser.current :>> ",
      otherUser.current
    );
    if (e.candidate) {
      otherUser.current.map((item) => {
        ...
        console.log(
          "2.1 handleICECandidateEvent userID :>> ",
  ...
  function handleNewICECandidateMsg(incoming) {
    ...
    console.log(
      "3. handleNewICECandidateMsg peerRef.current[userID] 1 :>> ",
      peerRef.current[userID]
    );
