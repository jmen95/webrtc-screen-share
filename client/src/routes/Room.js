...
class Room extends Component {
	...
	createSteam(stream) {
		...
		this.state.socketRef.on('disconnected', (userID) => {
			...
			const partnerVideos = this.state.partnerVideos.filter((item) => item.userID !== userID);
	...
	callUser(userID) {
		...
		this.state.userStream
			.getTracks()
			.forEach((track) =>
				this.state.senders.push(this.state.peerRef[userID].addTrack(track, this.state.userStream))
			);
	}
	...
	handleRecieveCall(incoming) {
		...
		this.state.peerRef[userID]
			...
			.then(() => {
				this.state.userStream
					.getTracks()
					.forEach((track) => this.state.peerRef[userID].addTrack(track, this.state.userStream));
			})
			.then(() => {
	...
	handleAnswer(message) {
		...
		console.log('2. this.handleAnswer this.state.peerRef :>> ', this.state.peerRef[userID]);
		console.log('2. this.handleAnswer this.state.peerCon :>> ', this.state.peerCon[userID]);
		console.log('2. this.handleAnswer userID :>> ', userID);
		if (!this.state.peerRef[userID]) {
			this.state.peerRef[userID] = this.createPeer(userID);
		}
		if (this.state.peerRef[userID])
			this.state.peerRef[userID].setRemoteDescription(desc).catch((e) => console.log(e));
		else {
		}
	...
	handleICECandidateEvent(e, userID) {
		console.log('2.1 this.handleICECandidateEvent this.state.otherUser :>> ', this.state.otherUser);
		if (e.candidate) {
			this.state.otherUser.map((item) => {
				console.log('2.1 this.handleICECandidateEvent item :>> ', item);
				console.log('2.1 this.handleICECandidateEvent userID :>> ', this.state.socketRef.id);
				const payload = {
					caller: this.state.socketRef.id,
					target: item,
	...
	handleNewICECandidateMsg(incoming) {
		...
		console.log('3. this.handleNewICECandidateMsg this.state.peerRef[userID] 1 :>> ', this.state.peerRef[userID]);
		...
		if (this.state.peerRef[userID]) {
			this.state.peerRef[userID].addIceCandidate(candidate).catch((e) => console.log(e));
		} else {
	...
	handleTrackEvent(userID) {
		return async function (e) {
			...
			const finalPartnerVideos = partnerVideos.filter((item) => item.userID !== userID);
			console.log('4 >>>>>> partnerVideos', partnerVideos);
			this.setState({
				// partnerVideos,
				partnerVideos: [...finalPartnerVideos, { userID, stream: e.streams[0] }],
			});
		}.bind(this);
	}
	...
	shareScreen() {
		navigator.mediaDevices.getDisplayMedia({ cursor: true }).then((stream) => {
			...
			this.state.senders.find((sender) => sender.track.kind === 'video').replaceTrack(screenTrack);
	...
	render() {
		...
		console.log('RENDER =>>>>>>>>>>>>>> this.state.partnerVideos', this.state.partnerVideos);
		return (
			<div>
