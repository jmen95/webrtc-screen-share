...
const Room = (props) => {
  ...
  function handleAnswer(message) {
    ...
    console.log("handleAnswer peerRef.current :>> ", peerRef.current[userID]);
    ...
    console.log(
      "handleAnswer peerRef.current[userID] :>> ",
      peerRef.current[userID]
    );
  }

  function handleICECandidateEvent(e, userID) {
    console.log(" handleICECandidateEvent userID :>> ", userID);
    if (e.candidate) {
  ...
  function handleNewICECandidateMsg(incoming) {
    ...
    console.log(
      "handleNewICECandidateMsg peerRef.current :>> ",
      peerRef.current
    );
    console.log("peerRef.current[userID] 1 :>> ", peerRef.current[userID]);
    if (!peerRef.current[userID]) {
      peerRef.current[userID] = createPeer(userID);
    }
